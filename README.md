# Document Converter Workspace (v1.8.2)
...
➡️ [**Download Document Converter (v1.8.2) for Windows**](https://github.com/duyphan1410/DocumentConvertTool/releases/latest)
...
```
The standalone desktop bundle will be generated at `dist/Document Converter/` and the Windows installer at `dist/installer/Document_Converter_Setup_v1.8.2.exe`.
...
- **v1.8.0 / v1.8.1 — Studio Workspace, Multi-Tab Architecture, Batch & Archive Converter**: Full IDE-grade transformation with Activity Bar dock, recursive File Explorer tree with inline filter, Quick Open fuzzy search modal (`Ctrl+P`), 60fps Dual Draggable Splitters, 2x2 Welcome Screen, floating right-click Context Menu, Win32 `SHFileOperationW` Recycle Bin deletion, 2-Tier Quick Convert (8 export formats), Multi-Tab Workspace (`ft.Draggable` tabs, `tab_session.json` isolated drafts, 0ms RAM preview, Atomic UI concurrency protection), Tab Context Menu & 3px micro scrollbar, Smart Drag & Drop (insert link/image into editor, move files in explorer), and multi-threaded Batch & Archive Converter (`.zip`, `.rar`, `.7z`, `.tar.gz`).
- **v1.8.2 — Floating Image Toolbar & Formatting, Multi-Tab Footer Sync, Explorer UX Polish (Current)**: Explorer-style image context menu & floating toolbar, custom image sizing dialog with responsive height/padding and click-outside dismiss, multi-tab conversion state & per-tab footer sync (`Open File` / `Open Location`), Word (`.docx`) image alignment engine (`WD_ALIGN_PARAGRAPH`), fixed keyboard shortcut `Ctrl+B` toggle, Explorer scrolling & boundary clipping fix (`ClipBehavior.HARD_EDGE`), smart recursive expanded folder state preservation on Refresh (`restore_expanded`), and full folder zone drag-and-drop.
