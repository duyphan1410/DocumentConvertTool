...
| **PowerPoint Module (`.pptx`)** | `src/modules/pptx_module.py` | Chuyển đổi slide PowerPoint `.pptx` hai chiều với Markdown (`PPTX ↔ MD`), hỗ trợ slide 16:9 widescreen, trích xuất danh sách đánh số (Numbering), bảng biểu, biểu đồ kèm chú thích có padding và cơ chế tự động tách slide chống tràn nội dung. | ✅ Completed (v1.6) |
