...
| Giai đoạn | Nhiệm vụ chính | Chi tiết công việc | Người thực hiện |
| :---: | :--- | :--- | :---: |
| **Pha 1**<br>*(Tuần 1)* | **Orchestrator, Hardware Hub & 2-Layer Verification** | • Tạo nhánh `feat/duy-DDMMYYYY-whisper-model-hub`.<br>• Cài đặt & cấu hình `faster-whisper`, `ctranslate2`, `pynvml`.<br>• Viết module Hardware Detection (`pynvml`/`nvidia-smi` quét Driver, CUDA, VRAM, RAM, CPU).<br>• Xây dựng `First-time Quick Download Dialog` và `ModelHubDialog`.<br>• Xây dựng Download Manager + Quy trình Verify 2 lớp (File Hash & Runtime Test độc lập).<br>🏁 **Mốc bàn giao**: Hạ tầng tải/quản lý model và runtime load đã được test độc lập thành công. | 👤 **Duy Phan**<br>*(Huy làm nhiệm vụ/feature khác)* |
| **Pha 2**<br>*(Tuần 2)* | **Core AI Engine & Processing Pipeline** | • Xây dựng module `src/services/whisper_service.py` dựa trên model đã được tải sẵn ở Pha 1.<br>• Xây dựng bộ giải mã âm thanh từ `.mp4`, `.mkv`, `.mp3`, `.wav`, `.m4a`.<br>• Tích hợp VAD lọc tạp âm, băm câu tự nhiên và sinh mốc thời gian `[mm:ss]`.<br>• Format tài liệu Markdown chuẩn chỉnh có cấu trúc. | 👤 **Huy**<br>*(AI Specialist)* |
| **Pha 3**<br>*(Tuần 3)* | **Tích hợp Toàn diện (E2E), QA & Đóng gói v1.9.0** | • Kết nối Controller với `whisper_service.transcribe_async()`.<br>• Đưa kết quả nhận diện tự động mở thành Tab mới trong Studio Workspace.<br>• Tích hợp nút khởi chạy trên Ribbon Bar, Welcome View và Settings.<br>• Benchmark độ chính xác (WER), kiểm thử Zero GUI Freeze khi xử lý file nặng.<br>• Cập nhật `Document Converter.spec` (`hiddenimports`) và đóng gói bản phát hành `v1.9.0`. | 👥 **Duy Phan & Huy**<br>*(Phối hợp)* |
