...
class JSONModule(BaseDocumentModule):
    ...
    def _data_to_markdown(self, data: Any, level: int = 1) -> str:
        ...
        if isinstance(data, list):
            ...
            # List of primitives, nested lists, or mixed items
            lines = []
            for item in data:
            ...
            # Complex dictionary: output primitives first so they belong to root container
            parts = []
    ...
    def _markdown_to_data(self, content: str) -> Any:
        ...
        while i < len(lines):
            ...
            if header_match:
                ...
                # Pop stack while top has level >= h_level
    ...
    def _coerce_value(self, val: str) -> Any:
        ...
        # Check integer / float (avoid converting IP addresses, dates, or multiple dots)
        if re.match(r"^-?\d+$", val):
            try:
                return int(val)
            except ValueError:
